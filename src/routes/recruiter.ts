...
async function accessibleJobIds(user: { id: string; organizationId: string; role: string }): Promise<string[]> {
  if (isAdmin(user.role)) {
    const rows = await prisma.$queryRawUnsafe<{ id: string }[]>(`SELECT id FROM "Job" WHERE "organizationId"=$1`, user.organizationId);
    return rows.map(r => r.id);
  }
  const rows = await prisma.$queryRawUnsafe<{ id: string }[]>(`
    SELECT j.id FROM "Job" j JOIN recruiter_job_access a ON a.job_id=j.id
    WHERE a.recruiter_id=$1 AND j."organizationId"=$2
...
async function resolveAccessibleJob(user: any, jobId: string) {
  ...
  const rows = await prisma.$queryRawUnsafe<any[]>(`
    SELECT j.id, j."externalId", j.title, j.status, j.location,
           j."employmentType", j."workplaceType", j."postedAt", j."applyUrl", j.description,
           j.requirements, j.responsibilities, j."preferredQualifications",
           o.id AS "organizationId", o.name AS "organizationName"
    FROM "Job" j JOIN "Organization" o ON o.id=j."organizationId"
    WHERE j.id = ANY($1::uuid[]) AND (j.id::text=$2 OR COALESCE(j."externalId",'')=$2)
    LIMIT 1
  `, ids, jobId);
  return rows[0] ?? null;
}
...
router.get("/dashboard", async (req, res) => {
  try {
    ...
    const ids = await accessibleJobIds(user);
    if (!ids.length) return res.json({ success: true, data: { user, metrics: { activeJobs: 0, applications: 0, candidates: 0, interviews: 0 } } });
    ...
    return res.json({ success: true, data: { user, metrics: { activeJobs: Number(jobs[0]?.count ?? 0), applications: Number(applications[0]?.count ?? 0), candidates: Number(candidates[0]?.count ?? 0), interviews: Number(interviews[0]?.count ?? 0) } } });
...
router.get("/jobs", async (req, res) => {
  try {
    const user = req.recruiter!;
    const ids = await accessibleJobIds(user);
    if (!ids.length) return res.json({ success: true, data: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0, hasMore: false } });
    ...
    const jobs = await prisma.$queryRawUnsafe<any[]>(`
      SELECT j.id, j."externalId", j.title, j.status, j.location, j.country, j.city,
             j."employmentType", j."workplaceType", j.remote, j."postedAt", j."expiresAt", j."applyUrl",
             j."canonicalUrl", o.id AS "organizationId", o.name AS "organizationName",
...
router.get("/candidates", async (req, res) => {
  try {
    ...
    const candidates = await prisma.$queryRawUnsafe<any[]>(`
      SELECT DISTINCT ON (c.id) c.id, c."firstName", c."lastName", c.email, c.phone, c."currentTitle",
             c.location, c.country, c.city, c."yearsExperience", c."linkedinUrl", c.status,
             c."createdAt", c."updatedAt"
      FROM "Candidate" c JOIN "Application" a ON a."candidateId"=c.id
      WHERE a."jobId"=ANY($1::uuid[])
        AND ($2='' OR c."firstName" ILIKE '%'||$2||'%' OR c."lastName" ILIKE '%'||$2||'%' OR c.email ILIKE '%'||$2||'%' OR c."currentTitle" ILIKE '%'||$2||'%')
...
router.get("/submissions", async (req, res) => {
  try {
    ...
    const search = typeof req.query.search === "string" ? req.query.search.trim() : "";
    const rows = await prisma.$queryRawUnsafe<any[]>(`
      SELECT a.id, a.status, a.source, a."submittedAt", a."createdAt",
             c.id AS "candidateId", c."firstName", c."lastName", c.email, c."currentTitle", c.location, c."linkedinUrl",
             j.id AS "jobId", j."externalId" AS "jobExternalId", j.title AS "jobTitle", o.name AS "organizationName"
      FROM "Application" a JOIN "Candidate" c ON c.id=a."candidateId" JOIN "Job" j ON j.id=a."jobId"
      JOIN "Organization" o ON o.id=j."organizationId"
      WHERE j.id=ANY($1::uuid[])
        AND ($2='' OR a.status::text=$2)
        AND ($3='' OR j.id::text=$3 OR COALESCE(j."externalId",'')=$3)
        AND ($4='' OR c."firstName" ILIKE '%'||$4||'%' OR c."lastName" ILIKE '%'||$4||'%' OR c.email ILIKE '%'||$4||'%' OR j.title ILIKE '%'||$4||'%')
      ORDER BY COALESCE(a."submittedAt",a."createdAt") DESC LIMIT 100
