...
export function buildHtnAtsSubmissionPayload(application: ApplicationWithRelations, input: CreateApplicationInput, job: HtnAtsSubmissionJob, recruiter: RecruiterIdentity) {
  ...
  const optionalCandidateFields: Array<[string, unknown]> = [
    ["phone", candidate.phone], ["location", candidate.location], ["currentCompany", input.currentCompany], ["currentTitle", candidate.currentTitle],
    ["yearsExperience", candidate.yearsExperience], ["linkedInUrl", candidate.linkedinUrl], ["desiredSalary", application.salaryExpectation], ["additionalNotes", application.additionalNotes],
  ];
  ...
  if (resume) {
    payload.resume = { fileName: resume.fileName, mimeType: resume.mimeType, sizeBytes: resume.size, storageProvider: "CLOUDFLARE_R2", storageKey: resume.storageKey };
  }
  return payload;
}

export async function syncHtnSubmissionToAts(application: ApplicationWithRelations, input: CreateApplicationInput, job: HtnAtsSubmissionJob, recruiter: RecruiterIdentity) {
  if (input.source !== ApplicationSource.RECRUITER) throw new AppError("VALIDATION_ERROR", "HTN ATS sync requires recruiter source", 400);
  const response = await fetch(`${getAtsBaseUrl()}/api/integrations/htn/submissions`, {
    method: "POST",
    headers: { Authorization: `Bearer ${getIntegrationKey()}`, "Content-Type": "application/json", Accept: "application/json" },
  ...
  if (!response.ok) {
    console.error("HTN to ATS submission sync failed", { status: response.status, candidateId: application.candidate.id, applicationId: application.id, jobId: job.id });
  ...
  if (!body?.success || !body?.candidateId || !body?.applicationId || !body?.jobId) {
    console.error("HTN to ATS submission sync returned invalid response", { candidateId: application.candidate.id, applicationId: application.id, jobId: job.id });
    throw new AppError("ATS_SYNC_FAILED", "ATS returned an invalid submission response", 502);
  }
  return body as { success: true; candidateId: string; applicationId: string; jobId: string; candidate: "created" | "updated"; application: "created" | "existing" };
}
